...
public class EditCompanyViewModel extends ViewModel {
    ...
    public void updateCompany(UpdateCompanyDTO updateCompanyDTO){
        int accountId=tokenManager.getAccountId();
        clientUtils.getUserService().updateCompany(accountId,updateCompanyDTO).enqueue(new Callback<UpdatedCompanyDTO>() {
            @Override
            public void onResponse(Call<UpdatedCompanyDTO> call, Response<UpdatedCompanyDTO> response) {
                if (response.isSuccessful() && response.body() != null) {
                    successMessage.setValue("Successfully updated company details!");
                } else {
                    error.setValue("Failed to update company details");
                }
            }

            @Override
            public void onFailure(Call<UpdatedCompanyDTO> call, Throwable t) {
                error.setValue(t.getMessage() != null ? t.getMessage() : "Error updating company details");
            }
        });
    }
}
